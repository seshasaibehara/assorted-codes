...
std::vector<Matrix3d> calc_l_primes(const std::vector<Vector3d>& grid_points)
{
    ...
    for (auto& x : grid_points)
    {
        for (auto& y : grid_points)
        {
            ...
            {
                lprime << x(0), y(0), z(0), x(1), y(1), z(1), x(2), y(2), z(2);
...
{
    if (compare_doubles(possible_sym_op.determinant(), 1, PREC, PREC) == true ||
        compare_doubles(possible_sym_op.determinant(), -1, PREC, PREC) == true)
    {
...
std::string label_sym_ops(sym_op_class sym_op_in)
{
    ...
    if (compare_doubles(det, -1, PREC, PREC) == true && compare_doubles(trace, -3, PREC, PREC) == false &&
        eigen_values == 0)
    {
...
MatrixXi generate_mult_table(const std::vector<Matrix3d>& point_group)
{
    ...
    for (int i = 0; i < order; i++)
    {
        for (int j = 0; j < order; j++)
        {
            ...
            auto it_k = std::find_if(point_group.begin(), point_group.end(), [&](const Matrix3d& symop) {
                return symop.isApprox(product, PREC);
            });
...
{
    if (compare_doubles(v0(0), v1(0), PREC, PREC) == true && compare_doubles(v0(1), v1(1), PREC, PREC) == true &&
        compare_doubles(v0(2), v1(2), PREC, PREC))
    {
        return true;
    }
...
std::vector<basis_class> generate_shifted_basis(const std::vector<basis_class>& new_basis,
                                                Vector3d possible_translation, Matrix3d lattice)
{
...
std::vector<Vector3d> generate_translations(const std::vector<basis_class>& new_basis,
                                            const std::vector<basis_class>& old_basis, const Matrix3d lattice)
{
